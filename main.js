        ...
        db.ref('locations').on('value', (snapshot) => {
          ...
          if (data) {
            ...
            for (const userId in data) {
              ...
              // Ganti warna marker jika lokasi palsu
              if (isFake) {
                ...
                markers[userId].setIcon(redIcon);
                ...
                markers[userId].setIcon(blueIcon);
                markers[userId].bindPopup(`<b>${userId}</b><br>Lokasi Asli<br>Alamat: ${address}<br>Waktu: ${timestamp}`).openPopup();
              }
              
              // Tambahkan ke daftar status
              const statusClass = isFake ? 'fake' : 'normal';
              const statusText = isFake ? 'LOKASI PALSU' : 'Lokasi Asli';
              listHtml += `<div class="employee-status"><span class="${statusClass}">●</span> <b>${userId}</b>: ${statusText}<br><small>Waktu: ${timestamp}</small></div>`;
            }
